...
//		cell = sheet.getWritableCell(7, 11);
...
//		cell = sheet.getWritableCell(1, 10);
		...
		addressList.add("Test");
		addressList.add("Test");
		fillRows(sheet, rowStart, rowEnd, colAddress, addressList);
		
		ArrayList<Float> distanceList = new ArrayList<Float>();
		distanceList.add(10.0f);
		distanceList.add(10.0f);
		distanceList.add(10.0f);
		distanceList.add(10.0f);
		fillRows(sheet, rowStart, rowEnd, colDistance, distanceList);
		
		ArrayList<Date> dateList = new ArrayList<Date>();
		dateList.add(new Date());
		fillRows(sheet, rowStart, rowEnd, colDate, dateList);
...
//				try {
//					// Change the hyperlink that begins in cell B40 to be a
//					// different API
//					wh.setURL(new URL(
...
//				wh.setFile(new File(
//						"d:/home/jexcelapi/docs/jxl/package-summary.html"));
...
//
//		wi = new WritableImage(1, 116, 2, 9, new File(
//				"resources/littlemoretonhall.png"));
//		sheet.addImage(wi);
//
...
//				botright.getColumn() - cell.getColumn(), 1);// botright.getRow()
//															// - cell.getRow());
	}

	private <T> void fillRows(WritableSheet sheet, int rowStart, int rowEnd,
			int col, List<T> list) throws WriteException, RowsExceededException {
		Label l;
		Number n;
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (val instanceof Date) {
				...
				String dateString = format.format((Date) val);
				l = new Label(col, row, dateString);
				sheet.addCell(l);
...
//				sheet.addCell(dt);
			...
			if (val instanceof String) {
				l = new Label(col, row, (String) val);
				sheet.addCell(l);
			} else if (val instanceof Float) {
				n = new Number(col, row, (Float) val);
				sheet.addCell(n);
			}
		}
	}
